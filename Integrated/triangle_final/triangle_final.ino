...
int state = 0;
//all cars other than the first set to false
boolean first_car = false;
//fill this with the bt addr of next car
String next_addr = ""; 
...
void setup() {//setting input and outputs
  ...
  //--------------------------- BLUETOOTH
  //set as slave if not the first car, blocked until master pairs
  
  //pull Enable pin (connected to Arduino pin #7) to high to prepare for entering AT slave mode
  digitalWrite(7,LOW);
  delay(2000);
  digitalWrite(7,HIGH);
  ...
  if(!first_car){
      ...
      //blocked until being paired by master
      while(digitalRead(4)!= 1){
        }
    }
    
  //pull Enable pin (connected to Arduino pin #7) to high to prepare for entering AT master mode
  digitalWrite(7,LOW);
  delay(2000);
  digitalWrite(7,HIGH);
  ...
  //bt warmup: may not be detecting signal rightaway, warmup rounds
...
void loop() {
    ...
    //state 1: arrived + master pairing slave 
    if (state == 1){
        Serial.print("Arrived");
        stopCar();
        pair(next_addr);
        state = 2; //nonexistent state
        Serial.println("Master shuts down");
    }
}// end of loop
...
void pair(String address){ // Master runs this when it gets close
  //constantly trying to pair with slave
  ...
  if(digitalRead(4)==1){
    ...
    delay(2000);
    digitalWrite(7,HIGH);
    //makesure that the master no longer has the iac 30 
    digitalWrite("AT+IAC=9e8b33\r\n");
  }
}
//pairing--------------------------------------
